...
#
	...
	Filter_vector = np.empty([34, 2], dtype=object)
	...
	Filter_vector[21] = [str('LCO'), str('g_lco')]
	Filter_vector[22] = [str('LCO'), str('i_lco')]
	Filter_vector[23] = [str('LCO'), str('z_lco')]
	Filter_vector[24] = [str('LCO'), str('r_lco')]
	Filter_vector[25] = [str('LCO'), str('u_lco')]
	Filter_vector[26] = [str('LCO'), str('U_lco')]
	Filter_vector[27] = [str('LCO'), str('B_lco')]
	Filter_vector[28] = [str('LCO'), str('V_lco')]
	Filter_vector[29] = [str('LT'), str('g_LT')]
	Filter_vector[30] = [str('LT'), str('i_LT')]
	Filter_vector[31] = [str('LT'), str('z_LT')]
	Filter_vector[32] = [str('LT'), str('r_LT')]
	Filter_vector[33] = [str('LT'), str('u_LT')]
		...
		for i,j in enumerate(Spectra): #goes through all epochs
			...
			if 'u_sdss' in j.keys():
				Spectrum.append(np.array(['sdss', 'u_sdss', j['u_sdss'],
										 ...
										 dtype=object))
			if 'r_LT' in j.keys():
				Spectrum.append(np.array(['LT', 'r_LT',j['r_LT'],max(j['r_LT_err'][1]-j['r_LT'],j['r_LT']-j['r_LT_err'][0])],dtype=object))
			if 'g_LT' in j.keys():
				Spectrum.append(np.array(['LT', 'g_LT',j['g_LT'],max(j['g_LT_err'][1]-j['g_LT'],j['g_LT']-j['g_LT_err'][0])],dtype=object))
			if 'i_LT' in j.keys():
				Spectrum.append(np.array(['LT', 'i_LT',j['i_LT'],max(j['i_LT_err'][1]-j['i_LT'],j['i_LT']-j['i_LT_err'][0])],dtype=object))
			if 'z_LT' in j.keys():
				Spectrum.append(np.array(['LT', 'z_LT',j['z_LT'],max(j['z_LT_err'][1]-j['z_LT'],j['z_LT']-j['z_LT_err'][0])],dtype=object))
			if 'u_LT' in j.keys():
				Spectrum.append(np.array(['LT', 'u_LT', j['u_LT'],
										  max(j['u_LT_err'][1] - j['u_LT'], j['u_LT'] - j['u_LT_err'][0])],
										 dtype=object))
			if 'U_lco' in j.keys():
				Spectrum.append(np.array(['LCO', 'U_lco',j['U_lco'],max(j['U_lco_err'][1]-j['U_lco'],j['U_lco']-j['U_lco_err'][0])],dtype=object))
			if 'B_lco' in j.keys():
				Spectrum.append(np.array(['LCO', 'B_lco',j['B_lco'],max(j['B_lco_err'][1]-j['B_lco'],j['B_lco']-j['B_lco_err'][0])],dtype=object))
			if 'V_lco' in j.keys():
				Spectrum.append(np.array(['LCO', 'V_lco',j['V_lco'],max(j['V_lco_err'][1]-j['V_lco'],j['V_lco']-j['V_lco_err'][0])],dtype=object))
			if 'r_lco' in j.keys():
				Spectrum.append(np.array(['LCO', 'r_lco',j['r_lco'],max(j['r_lco_err'][1]-j['r_lco'],j['r_lco']-j['r_lco_err'][0])],dtype=object))
			if 'g_lco' in j.keys():
				Spectrum.append(np.array(['lco', 'g_lco',j['g_lco'],max(j['g_lco_err'][1]-j['g_lco'],j['g_lco']-j['g_lco_err'][0])],dtype=object))
			if 'i_lco' in j.keys():
				Spectrum.append(np.array(['lco', 'i_lco',j['i_lco'],max(j['i_lco_err'][1]-j['i_lco'],j['i_lco']-j['i_lco_err'][0])],dtype=object))
			if 'z_lco' in j.keys():
				Spectrum.append(np.array(['lco', 'z_lco',j['z_lco'],max(j['z_lco_err'][1]-j['z_lco'],j['z_lco']-j['z_lco_err'][0])],dtype=object))
			if 'u_lco' in j.keys():
				Spectrum.append(np.array(['lco', 'u_lco', j['u_lco'],
										  max(j['u_lco_err'][1] - j['u_lco'], j['u_lco'] - j['u_lco_err'][0])],
										 dtype=object))
			...
			if mcmc==True:
				...
				if already_run_fit[i]==False:
					...
					if priors is False:
						...
						already_run_dynesty=already_run_fit[i],already_run_calc_all_chis=False,priors=p,
				...
				if already_run_fit[i]==False:
					if priors is False:
						[Xi_array, best_temp, index_min, best_coeff, best_radius,best_luminosity]=fit_black_body_flux_filters.fit_black_body_flux_filters(
							...
							,path_to_txt_file=output+'/day_'+str(round(j['time'],3))+'/best_fit_results.txt',filters_directory=filters_directory)
					else:
						[Xi_array, best_temp, index_min, best_coeff, best_radius,best_luminosity]=fit_black_body_flux_filters.fit_black_body_flux_filters(
					...
					best_temp=np.genfromtxt(output+'/day_'+str(round(j['time'],3))+'/best_fit_results.txt')[1]
					best_radius=np.genfromtxt(output+'/day_'+str(round(j['time'],3))+'/best_fit_results.txt')[4]
					best_luminosity=np.genfromtxt(output+'/day_'+str(round(j['time'],3))+'/best_fit_results.txt')[5]
...
def plot_L_in_time(Best,dates_file=None,data_file=None,lower_limit_on_flux=None,error_lum_ran=False,explosion_date=None,output=None,mcmc=False, dynesty=False,output_file_interpolation=None,ylim=None,excluded_bands=[],verbose=False):
	...
	if (mcmc==True)|(dynesty==True):
		...
		already_run_interp_errors['r_LT'] = True
		already_run_interp_errors['g_LT'] = True
		already_run_interp_errors['i_LT'] = True
		already_run_interp_errors['u_LT'] = True
		already_run_interp_errors['z_LT'] = True
		already_run_interp_errors['r_lco'] = True
		already_run_interp_errors['g_lco'] = True
		already_run_interp_errors['i_lco'] = True
		already_run_interp_errors['u_lco'] = True
		already_run_interp_errors['z_lco'] = True
		already_run_interp_errors['U_lco'] = True
		already_run_interp_errors['V_lco'] = True
		already_run_interp_errors['B_lco'] = True
		...
		for i, j in enumerate(interpolation_dates):
			...
			for k in [x for x in data_dicts.keys() if x not in excluded_bands]:
				if len(JD_basis_interpolation[k]) > 0:
					if str(epoch['time']) in interp[k].keys():
						epoch[k] = interp[k][str(epoch['time'])].flatten()[1]  # flux
						epoch[k + '_err'] = [interp[k][str(epoch['time'])].flatten()[3], interp[k][str(epoch['time'])].flatten()[4]]  # fluxerr
			Spectra.append(epoch)
		...
		if number>=4:
			...
			for i, row in enumerate(Spectra2D):
				for k, j in enumerate(row):
					...
					if 'g_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['g_lco']], [j['g_lco']], color=color['g_lco'],marker=symbol['g_lco'],label='LCO g')  # ,label='P60 g')
						axes2d[k, i].vlines(wavelengths_filter['g_lco'], j['g_lco_err'][0], j['g_lco_err'][1])
						if 'g_lco' not in bands_in_legend:
							bands_in_legend.append('g_lco')
							order_in_legend.append('g_lco')
					if 'i_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['i_lco']], [j['i_lco']], color=color['i_lco'],marker=symbol['i_lco'],label='LCO i')  # ,label='P60 i')
						axes2d[k, i].vlines(wavelengths_filter['i_lco'], j['i_lco_err'][0], j['i_lco_err'][1])
						if 'i_lco' not in bands_in_legend:
							bands_in_legend.append('i_lco')
							order_in_legend.append('i_lco')
					if 'r_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['r_lco']], [j['r_lco']], color=color['r_lco'],marker=symbol['r_lco'],label='LCO r')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['r_lco'], j['r_lco_err'][0], j['r_lco_err'][1])
						if 'r_lco' not in bands_in_legend:
							bands_in_legend.append('r_lco')
							order_in_legend.append('r_lco')
					if 'z_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['z_lco']], [j['z_lco']], color=color['z_lco'],
										  marker=symbol['z_lco'], label='LCO z')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['z_lco'], j['z_lco_err'][0], j['z_lco_err'][1])
						if 'z_lco' not in bands_in_legend:
							bands_in_legend.append('z_lco')
							order_in_legend.append('z_lco')
					if 'u_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['u_lco']], [j['u_lco']], color=color['u_lco'],
										  marker=symbol['u_lco'], label='LCO u')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['u_lco'], j['u_lco_err'][0], j['u_lco_err'][1])
						if 'u_lco' not in bands_in_legend:
							bands_in_legend.append('u_lco')
							order_in_legend.append('u_lco')
					if 'U_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['U_lco']], [j['U_lco']], color=color['U_lco'],
										  marker=symbol['U_lco'], label='LCO U')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['U_lco'], j['U_lco_err'][0], j['U_lco_err'][1])
						if 'U_lco' not in bands_in_legend:
							...
							order_in_legend.append('U_lco')
					if 'B_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['B_lco']], [j['B_lco']], color=color['B_lco'],
										  marker=symbol['B_lco'], label='LCO B')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['B_lco'], j['B_lco_err'][0], j['B_lco_err'][1])
						if 'B_lco' not in bands_in_legend:
							bands_in_legend.append('B_lco')
							order_in_legend.append('B_lco')
					if 'V_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['V_lco']], [j['V_lco']], color=color['V_lco'],
										  marker=symbol['V_lco'], label='LCO V')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['V_lco'], j['V_lco_err'][0], j['V_lco_err'][1])
						if 'V_lco' not in bands_in_legend:
							bands_in_legend.append('V_lco')
							order_in_legend.append('V_lco')
					if 'g_LT' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['g_LT']], [j['g_LT']], color=color['g_LT'],marker=symbol['g_LT'],label='LT g')  # ,label='P60 g')
						axes2d[k, i].vlines(wavelengths_filter['g_LT'], j['g_LT_err'][0], j['g_LT_err'][1])
						if 'g_LT' not in bands_in_legend:
							bands_in_legend.append('g_LT')
							order_in_legend.append('g_LT')
					if 'i_LT' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['i_LT']], [j['i_LT']], color=color['i_LT'],marker=symbol['i_LT'],label='LT i')  # ,label='P60 i')
						axes2d[k, i].vlines(wavelengths_filter['i_LT'], j['i_LT_err'][0], j['i_LT_err'][1])
						if 'i_LT' not in bands_in_legend:
							bands_in_legend.append('i_LT')
							order_in_legend.append('i_LT')
					if 'r_LT' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['r_LT']], [j['r_LT']], color=color['r_LT'],marker=symbol['r_LT'],label='LT r')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['r_LT'], j['r_LT_err'][0], j['r_LT_err'][1])
						if 'r_LT' not in bands_in_legend:
							bands_in_legend.append('r_LT')
							order_in_legend.append('r_LT')
					if 'z_LT' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['z_LT']], [j['z_LT']], color=color['z_LT'],
										  marker=symbol['z_LT'], label='LT z')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['z_LT'], j['z_LT_err'][0], j['z_LT_err'][1])
						if 'z_LT' not in bands_in_legend:
							bands_in_legend.append('z_LT')
							order_in_legend.append('z_LT')
					if 'u_LT' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['u_LT']], [j['u_LT']], color=color['u_LT'],
										  marker=symbol['u_LT'], label='LT u')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['u_LT'], j['u_LT_err'][0], j['u_LT_err'][1])
						if 'u_LT' not in bands_in_legend:
							bands_in_legend.append('u_LT')
							order_in_legend.append('u_LT')
					if 'u_swift' in j.keys():
			...
			pylab.savefig(output + '/2D_SEDs_4.png',
							  facecolor='w', edgecolor='w', orientation='portrait', papertype=None, format='png', transparent=False,
				...
				for k, j in enumerate(row):
					...
					if 'u_sdss' in j.keys():
						...
						if 'u_sdss' not in bands_in_legend:
							bands_in_legend.append('u_sdss')
					if 'g_lco' in j.keys():
						axes2d[i].plot([wavelengths_filter['g_lco']], [j['g_lco']], color=color['g_lco'],
										  marker=symbol['g_lco'], label='LCO g')  # ,label='P60 g')
						axes2d[i].vlines(wavelengths_filter['g_lco'], j['g_lco_err'][0], j['g_lco_err'][1])
						if 'g_lco' not in bands_in_legend:
							bands_in_legend.append('g_lco')
					if 'i_lco' in j.keys():
						axes2d[i].plot([wavelengths_filter['i_lco']], [j['i_lco']], color=color['i_lco'],
										  marker=symbol['i_lco'], label='LCO i')  # ,label='P60 i')
						axes2d[i].vlines(wavelengths_filter['i_lco'], j['i_lco_err'][0], j['i_lco_err'][1])
						if 'i_lco' not in bands_in_legend:
							bands_in_legend.append('i_lco')
					if 'r_lco' in j.keys():
						axes2d[i].plot([wavelengths_filter['r_lco']], [j['r_lco']], color=color['r_lco'],
										  marker=symbol['r_lco'], label='LCO r')  # ,label='P60 r')
						axes2d[i].vlines(wavelengths_filter['r_lco'], j['r_lco_err'][0], j['r_lco_err'][1])
						if 'r_lco' not in bands_in_legend:
							bands_in_legend.append('r_lco')
					if 'z_lco' in j.keys():
						axes2d[i].plot([wavelengths_filter['z_lco']], [j['z_lco']], color=color['z_lco'],
										  marker=symbol['z_lco'], label='LCO z')  # ,label='P60 r')
						axes2d[i].vlines(wavelengths_filter['z_lco'], j['z_lco_err'][0], j['z_lco_err'][1])
						if 'z_lco' not in bands_in_legend:
							bands_in_legend.append('z_lco')
					if 'u_lco' in j.keys():
						axes2d[i].plot([wavelengths_filter['u_lco']], [j['u_lco']], color=color['u_lco'],
										  marker=symbol['u_lco'], label='LCO u')  # ,label='P60 r')
						axes2d[i].vlines(wavelengths_filter['u_lco'], j['u_lco_err'][0], j['u_lco_err'][1])
						if 'u_lco' not in bands_in_legend:
							bands_in_legend.append('u_lco')
					if 'U_lco' in j.keys():
						axes2d[i].plot([wavelengths_filter['U_lco']], [j['U_lco']], color=color['U_lco'],
						...
						if 'U_lco' not in bands_in_legend:
							bands_in_legend.append('U_lco')
					if 'B_lco' in j.keys():
						axes2d[i].plot([wavelengths_filter['B_lco']], [j['B_lco']], color=color['B_lco'],
										  marker=symbol['B_lco'], label='LCO B')  # ,label='P60 r')
						axes2d[i].vlines(wavelengths_filter['B_lco'], j['B_lco_err'][0], j['B_lco_err'][1])
						if 'B_lco' not in bands_in_legend:
							bands_in_legend.append('B_lco')
					if 'V_lco' in j.keys():
						axes2d[i].plot([wavelengths_filter['V_lco']], [j['V_lco']], color=color['V_lco'],
										  marker=symbol['V_lco'], label='LCO V')  # ,label='P60 r')
						axes2d[i].vlines(wavelengths_filter['V_lco'], j['V_lco_err'][0], j['V_lco_err'][1])
						if 'V_lco' not in bands_in_legend:
							bands_in_legend.append('V_lco')
					if 'g_LT' in j.keys():
						axes2d[i].plot([wavelengths_filter['g_LT']], [j['g_LT']], color=color['g_LT'],
										  marker=symbol['g_LT'], label='LT g')  # ,label='P60 g')
						axes2d[i].vlines(wavelengths_filter['g_LT'], j['g_LT_err'][0], j['g_LT_err'][1])
						if 'g_LT' not in bands_in_legend:
							bands_in_legend.append('g_LT')
					if 'i_LT' in j.keys():
						axes2d[i].plot([wavelengths_filter['i_LT']], [j['i_LT']], color=color['i_LT'],
										  marker=symbol['i_LT'], label='LT i')  # ,label='P60 i')
						axes2d[i].vlines(wavelengths_filter['i_LT'], j['i_LT_err'][0], j['i_LT_err'][1])
						if 'i_LT' not in bands_in_legend:
							bands_in_legend.append('i_LT')
					if 'r_LT' in j.keys():
						axes2d[i].plot([wavelengths_filter['r_LT']], [j['r_LT']], color=color['r_LT'],
										  marker=symbol['r_LT'], label='LT r')  # ,label='P60 r')
						axes2d[i].vlines(wavelengths_filter['r_LT'], j['r_LT_err'][0], j['r_LT_err'][1])
						if 'r_LT' not in bands_in_legend:
							bands_in_legend.append('r_LT')
					if 'z_LT' in j.keys():
						axes2d[i].plot([wavelengths_filter['z_LT']], [j['z_LT']], color=color['z_LT'],
										  marker=symbol['z_LT'], label='LT z')  # ,label='P60 r')
						axes2d[i].vlines(wavelengths_filter['z_LT'], j['z_LT_err'][0], j['z_LT_err'][1])
						if 'z_LT' not in bands_in_legend:
							bands_in_legend.append('z_LT')
					if 'u_LT' in j.keys():
						axes2d[i].plot([wavelengths_filter['u_LT']], [j['u_LT']], color=color['u_LT'],
										  marker=symbol['u_LT'], label='LT u')  # ,label='P60 r')
						axes2d[i].vlines(wavelengths_filter['u_LT'], j['u_LT_err'][0], j['u_LT_err'][1])
						if 'u_LT' not in bands_in_legend:
							bands_in_legend.append('u_LT')
					if 'u_swift' in j.keys():
			...
			fig.subplots_adjust(left=0.2)
			pylab.savefig(output + '/2D_SEDs_4.png',facecolor='w', edgecolor='w', orientation='portrait', papertype=None, format='png',
						  transparent=False,
						  bbox_inches=None, pad_inches=1.5)
			plt.show()

	elif number_of_plot==9:
		number=0
		number=len(open(dates_file).readlines())
			...
			for i, row in enumerate(Spectra2D):
				for k, j in enumerate(row):
					...
					if 'g_lco' in j.keys():
						...
						if 'g_lco' not in bands_in_legend:
							bands_in_legend.append('g_lco')
							order_in_legend.append(order_bands['g_lco'])
					if 'i_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['i_lco']], [j['i_lco']], color=color['i_lco'],marker=symbol['i_lco'],label='LCO i')  # ,label='P60 i')
						axes2d[k, i].vlines(wavelengths_filter['i_lco'], j['i_lco_err'][0], j['i_lco_err'][1])
						if 'i_lco' not in bands_in_legend:
							bands_in_legend.append('i_lco')
							order_in_legend.append(order_bands['i_lco'])
					if 'r_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['r_lco']], [j['r_lco']], color=color['r_lco'],marker=symbol['r_lco'],label='LCO r')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['r_lco'], j['r_lco_err'][0], j['r_lco_err'][1])
						if 'r_lco' not in bands_in_legend:
							bands_in_legend.append('r_lco')
							order_in_legend.append(order_bands['r_lco'])
					if 'z_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['z_lco']], [j['z_lco']], color=color['z_lco'],
										  marker=symbol['z_lco'], label='LCO z')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['z_lco'], j['z_lco_err'][0], j['z_lco_err'][1])
						if 'z_lco' not in bands_in_legend:
							bands_in_legend.append('z_lco')
							order_in_legend.append(order_bands['z_lco'])
					if 'u_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['u_lco']], [j['u_lco']], color=color['u_lco'],
										  marker=symbol['u_lco'], label='LCO u')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['u_lco'], j['u_lco_err'][0], j['u_lco_err'][1])
						if 'u_lco' not in bands_in_legend:
							bands_in_legend.append('u_lco')
							order_in_legend.append(order_bands['u_lco'])
					if 'U_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['U_lco']], [j['U_lco']], color=color['U_lco'],
										  marker=symbol['U_lco'], label='LCO U')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['U_lco'], j['U_lco_err'][0], j['U_lco_err'][1])
					...
					if 'B_lco' in j.keys():
						...
						if 'B_lco' not in bands_in_legend:
							bands_in_legend.append('B_lco')
							order_in_legend.append(order_bands['B_lco'])
					if 'V_lco' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['V_lco']], [j['V_lco']], color=color['V_lco'],
										  marker=symbol['V_lco'], label='LCO V')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['V_lco'], j['V_lco_err'][0], j['V_lco_err'][1])
					...
					if 'g_LT' in j.keys():
						...
						if 'g_LT' not in bands_in_legend:
							bands_in_legend.append('g_LT')
							order_in_legend.append(order_bands['g_LT'])
					if 'i_LT' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['i_LT']], [j['i_LT']], color=color['i_LT'],marker=symbol['i_LT'],label='LT i')  # ,label='P60 i')
						axes2d[k, i].vlines(wavelengths_filter['i_LT'], j['i_LT_err'][0], j['i_LT_err'][1])
						if 'i_LT' not in bands_in_legend:
							bands_in_legend.append('i_LT')
							order_in_legend.append(order_bands['i_LT'])
					if 'r_LT' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['r_LT']], [j['r_LT']], color=color['r_LT'],marker=symbol['r_LT'],label='LT r')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['r_LT'], j['r_LT_err'][0], j['r_LT_err'][1])
						if 'r_LT' not in bands_in_legend:
							bands_in_legend.append('r_LT')
							order_in_legend.append(order_bands['r_LT'])
					if 'z_LT' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['z_LT']], [j['z_LT']], color=color['z_LT'],
										  marker=symbol['z_LT'], label='LT z')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['z_LT'], j['z_LT_err'][0], j['z_LT_err'][1])
						if 'z_LT' not in bands_in_legend:
							bands_in_legend.append('z_LT')
							order_in_legend.append(order_bands['z_LT'])
					if 'u_LT' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['u_LT']], [j['u_LT']], color=color['u_LT'],
										  marker=symbol['u_LT'], label='LT u')  # ,label='P60 r')
						axes2d[k, i].vlines(wavelengths_filter['u_LT'], j['u_LT_err'][0], j['u_LT_err'][1])
						if 'u_LT' not in bands_in_legend:
							bands_in_legend.append('u_LT')
							order_in_legend.append(order_bands['u_LT'])
					if 'u_swift' in j.keys():
						axes2d[k, i].plot([wavelengths_filter['u_swift']], [j['u_swift']], color=color['u_swift'],marker=symbol['u_swift'],label='u')  # ,label='u')
						axes2d[k, i].vlines(wavelengths_filter['u_swift'], j['u_swift_err'][0], j['u_swift_err'][1])
						if 'u_swift' not in bands_in_legend:
